...
net = Net().cpu().eval()
net.load_state_dict(torch.load(parent_dir+os.sep+'sex_case8_yolocut.pt', map_location=torch.device('cpu')))

def predict(img):
    print("sexnet_resnet50 predict")
    img = Image.fromarray(img)
    # image = Image.open(img).convert("RGB")
    print("sexnet_resnet50 transform")
    print(str(img))
    data = None
    try:
        data = transform(img)
